    ...
    public:
        ...
        */
            ...
            MemoryPoolHandle pool = MemoryManager::GetPool())
        {
         ...
         /**
        Encodes a vector of double-precision floating-point real or complex numbers
        into a plaintext polynomial. Padding zeros if vector size is less than N/2.
        Dynamic memory allocations in the process are allocated from the memory
        pool pointed to by the given MemoryPoolHandle.
        ...
        */
        template <
            typename T, typename = std::enable_if_t<
                            std::is_same<std::remove_cv_t<T>, double>::value ||
                            std::is_same<std::remove_cv_t<T>, std::complex<double>>::value>>
        inline void paddingEncode(
            const std::vector<T> &values, parms_id_type parms_id, double scale, Plaintext &destination,
            MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            size_t stride = slots_ / values.size();
            std::vector<T> temp(slots_, 0);
            for (size_t i = 0; i != values.size(); ++i) temp[i * stride] = values[i];
            encode_internal(temp.data(), temp.size(), parms_id, scale, destination, std::move(pool));
        }
        ...
        */
            ...
            MemoryPoolHandle pool = MemoryManager::GetPool())
        {
            ...
            std::vector<T> temp(slots_, 0);
            for (size_t i = 0; i != values.size(); ++i) temp[i * stride] = values[i];
            encode(temp, context_.first_parms_id(), scale, destination, std::move(pool));
        }
#ifdef SEAL_USE_MSGSL
        /**
        Encodes a vector of double-precision floating-point real or complex numbers
